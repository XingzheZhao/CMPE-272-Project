...
router.get("/on-sale-items", async (req, res) => {
  try {
    ...
    let sql =
      'SELECT item_id, item_name, item_price, is_exchange, item_image, post_datetime FROM Item WHERE item_status = "on sale" ORDER BY post_datetime DESC';
...
router.get("/in-progress-items", async (req, res) => {
  try {
    ...
    let sql =
      'SELECT item_id, item_name, item_price, is_exchange, item_image, post_datetime FROM Item WHERE item_status = "progress" AND (buyer_id = ? OR seller_id = ?)';
...
router.get("/item", async (req, res) => {
  try {
    ...
    let sql =
      "SELECT I.item_name, I.item_type, I.item_price, I.is_exchange, I.exchange_demand, I.item_image, I.post_datetime, I.item_description, U.username, U.email FROM Item I, Users U WHERE U.user_id = I.seller_id AND I.item_id = ?";
...
router.post("/item/edit", upload.single("item_image"), async (req, res) => {
  try {
    ...
    const sql =
      "UPDATE Item SET item_name = ?, item_type = ?, item_description = ?, item_image = ?, is_exchange = ?, item_price = ?, exchange_demand = ? WHERE item_id = ?";
    db.query(
      sql,
      [
        item_name,
...
router.post(
  "/item/edit-null-image",
  upload.single("item_image"),
  async (req, res) => {
    try {
      const db = require("../index").db;
      const { id, item_name, item_description, item_price, item_type } =
        req.body;
      ...
      const sql =
        "UPDATE Item SET item_name = ?, item_type = ?, item_description = ?, item_image = NULL, is_exchange = ?, item_price = ?, exchange_demand = ? WHERE item_id = ?";
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const sql =
        "UPDATE Item SET item_name = ?, item_type = ?, item_description = ?, is_exchange = ?, item_price = ?, exchange_demand = ? WHERE item_id = ?";
      db.query(
        sql,
        [
          item_name,
...
router.post("/not-interest", async (req, res) => {
  try {
    ...
    const sql =
      "UPDATE Item SET item_status = ?, buyer_id = NULL WHERE item_id = ?";
...
router.post(
  ...
  async (req, res) => {
    ...
    const sql =
      "INSERT INTO Item (seller_id, item_name, item_type, item_price, is_exchange, exchange_demand, item_image, item_status, post_datetime, item_description) VALUES (?, ?, ?, ?, ?, ?, NULL, ?, NOW(), ?)";
    if (is_exchange === "true" || is_exchange === "1") {
      db.query(
        sql,
        [
...
router.post("/item/create", upload.single("item_image"), async (req, res) => {
  try {
    ...
    const sql =
      "INSERT INTO Item (seller_id, item_name, item_type, item_price, is_exchange, exchange_demand, item_image, item_status, post_datetime, item_description) VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW(), ?)";
    db.query(
      sql,
      [
        id,
